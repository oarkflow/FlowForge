...
const SettingsPage: Component = () => {
  ...
  const [slackNotifs, setSlackNotifs] = createSignal(true);
  const [digestMode, setDigestMode] = createSignal(false);
  ...
  const handleSaveNotifications = async () => {
    ...
    try {
      await apiClient.put('/users/me/notifications', {
          ...
          <Switch>
            ...
            <Match when={activeSection() === 'notifications'}>
              <Card title="Notification Preferences" description="Choose how you want to be notified">
                <div class="space-y-4 max-w-lg">
                  <For each={[
                    { label: 'Email Notifications', desc: 'Receive pipeline status updates via email', get: emailNotifs, set: setEmailNotifs },
                    { label: 'Slack Notifications', desc: 'Get alerts in connected Slack channels', get: slackNotifs, set: setSlackNotifs },
                    { label: 'Digest Mode', desc: 'Batch notifications into a daily summary', get: digestMode, set: setDigestMode },
                  ]}>
                    {(pref) => (
                      <div class="flex items-center justify-between p-4 rounded-lg bg-[var(--color-bg-secondary)] border border-[var(--color-border-primary)]">
                        <div>
                          ...
                          <p class="text-xs text-[var(--color-text-tertiary)] mt-0.5">{pref.desc}</p>
                        </div>
                        <button
                          class={`w-10 h-6 rounded-full relative cursor-pointer transition-colors ${pref.get() ? 'bg-indigo-500' : 'bg-gray-600'}`}
                          onClick={() => pref.set(!pref.get())}
                        >
                          <div class={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${pref.get() ? 'left-5' : 'left-1'}`} />
                        </button>
                      </div>
                    )}
                  </For>
                  ...
                  <div class="pt-2">
                    <h4 class="text-sm font-medium text-[var(--color-text-primary)] mb-3">Notify me when:</h4>
                    <div class="space-y-2">
                      <For each={[
                        'Pipeline run fails',
                        'Pipeline run succeeds (after failure)',
                        ...
                        {(event) => (
                          <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" checked class="w-4 h-4 rounded border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)] text-indigo-500 focus:ring-indigo-500/40" />
                            <span class="text-sm text-[var(--color-text-secondary)]">{event}</span>
                          </label>
                        )}
                      </For>
                    </div>
                        ...
                        {(t) => (
                          <button
                            class={`p-3 rounded-lg border-2 transition-colors ${
                              theme() === t.id ? 'border-indigo-500' : 'border-[var(--color-border-primary)] hover:border-[var(--color-border-secondary)]'
                            }`}
