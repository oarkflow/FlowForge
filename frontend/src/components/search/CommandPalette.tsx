...
const CommandPalette: Component<CommandPaletteProps> = (props) => {
	...
	const allItems = () => {
		const items: { type: string; label: string; sublabel: string; path: string }[] = [];
	...
	const hasResults = () => projects().length > 0 || pipelines().length > 0 || runs().length > 0;
				...
				<div class="w-full max-w-lg rounded-xl bg-[var(--color-bg-secondary)] border border-[var(--color-border-primary)] shadow-2xl overflow-hidden">
					...
					<div class="flex items-center gap-3 px-4 py-3 border-b border-[var(--color-border-primary)]">
						...
						<input
							ref={inputRef}
							type="text"
							placeholder="Search projects, pipelines, runs..."
					...
					<div class="max-h-[360px] overflow-y-auto">
						...
						{/* No results */}
						<Show when={hasQuery() && !loading() && !hasResults()}>
